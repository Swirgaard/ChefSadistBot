...
def get_main_menu_builder() -> InlineKeyboardBuilder:
    ...
    categories = [
        ("🔥 Горячее", "hot_dishes"), ("🥣 Супы", "soups"),
        ("🍝 Паста", "pasta"), ("🥗 Салаты", "salads"),
        ("🥔 Гарниры", "garnishes"), ("🍳 Завтраки", "breakfasts"),
        ("🥪 Бутерброды", "sandwiches"), ("✨ Жареное Золото", "fried_gold"),
        ("🥧 Выпечка", "baked_goods"), ("🍰 Десерты", "desserts"),
        ("🌶️ Соусы", "sauces"), ("🍸 Напитки", "drinks"),
...
async def send_recipe_response(message_or_callback: types.Message | types.CallbackQuery, response_data: dict):
    ...
    Теперь прикрепляет кнопку 'Назад' к самому рецепту.
    """
    ...
    # Основная логика отправки сообщения (рецепт или выбор)
    if reply_markup:
        await target_message.answer(response_text, reply_markup=reply_markup)
        return

    # Если это рецепт, создаем клавиатуру с терминами и кнопкой "Назад"
    builder = InlineKeyboardBuilder()
    if found_terms:
    ...
    # Добавляем контекстную кнопку "Назад"
    ...
    else: # 'main' or from manual search
...
async def show_main_menu(message_or_callback: types.Message | types.CallbackQuery, text: str):
    """Отображает главное меню категорий."""
    user_id = message_or_callback.from_user.id
    session = get_user_session(user_id)
...
async def start_command(message: types.Message):
    """Сбрасывает сессию и выдает главное меню."""
...
async def handle_ingredients(message: types.Message):
    # ... (логика без изменений, но send_recipe_response теперь работает по-новому)
    get_user_session(message.from_user.id)['last_menu'] = 'main'
    # ...
...
async def process_cuisine_callback(callback_query: types.CallbackQuery):
    ...
    # ... (остальная логика поиска рецепта и защиты от повторов остается прежней)
    # ...
