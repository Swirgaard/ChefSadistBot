...
def get_main_menu_builder() -> InlineKeyboardBuilder:
    ...
    categories = [
        ...
        ("🥧 Выпечка", "baked_goods"), # <-- НОВАЯ КНОПКА
        ("🍰 Десерты", "desserts"),
        ("🌶️ Соусы", "sauces"),
        ("🍸 Напитки", "drinks")
    ]
    ...
    builder.adjust(3) # Раскладка 3х4
    return builder
...
CATEGORY_REACTIONS = {
    ...
    "fried_gold": "А, это самое вредное... и самое вкусное! Смотри не сожги, а то будет не золото, а уголь.", # НОВАЯ РЕАКЦИЯ ДЛЯ ЖАРЕНОГО ЗОЛОТА
    "drinks": "Напитки? Хорошо. Главное — чтобы потом не пришлось вызывать 'Скорую помощь'. Ищи, что тебе по вкусу.", # НОВАЯ РЕАКЦИЯ ДЛЯ НАПИТКОВ
    "baked_goods": "Выпечка... Чудесно. Это то, что делает твою жизнь чуть слаще. И твою талию — чуть шире. Выбирай осторожно.", # НОВАЯ РЕАКЦИЯ ДЛЯ ВЫПЕЧКИ
}
