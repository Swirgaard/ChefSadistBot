...
def find_recipe_by_intention(query: str) -> dict | None:
    ...
    """
    # Приводим запрос пользователя к нижнему регистру и нормализуем пробелы
...
def find_recipe_by_id(recipe_id: str) -> dict | None:
    """
    Находит рецепт в базе знаний по его уникальному ID.
    Возвращает словарь с рецептом или None, если ничего не найдено.
    """
    recipes_db = KNOWLEDGE_BASE.get("recipes", [])
    for recipe in recipes_db:
...
def synthesize_response(user_query: str) -> Dict[str, Any]:
    ...
    if not found_ingredients:
        return {
            "text": phrases.get("rejection_phrases", {}).get("no_ingredients_found", "Я не нашла никаких знакомых ингредиентов. Попробуй перечислить их более конкретно или проверь свой холодильник. Возможно, там уже завелась разумная жизнь и сожрала все запасы."),
            "found_terms": []
        }
    ...
    elif match_result["status"] == "partial_options": # Теперь это обрабатывается здесь
        builder = InlineKeyboardBuilder()
        options_text_lines = []
        
        # Собираем текст для предложенных опций
        options_text_lines.append(phrases.get("rejection_phrases", {}).get("partial_match_options_intro", "Я не нашла идеального совпадения, но вот что можно попробовать:\n"))
        
        for i, p_info in enumerate(match_result["options"]):
            ...
            for key in p_info["excess_keys"]:
                excess_names.append(ingredients_db.get(key, {}).get("name_forms", {}).get("acc_sg", key))

            option_line = f"{i+1}. {recipe.get('title', 'Безымянный Протокол')} (есть: {p_info['match_count']}/{len(recipe.get('trigger_keys', []))}"
            if missing_names:
                option_line += f", не хватает: {', '.join(missing_names)}"
            if excess_names: # Вывод лишних ингредиентов, если они есть
                option_line += f", лишние: {', '.join(excess_names)}"
            option_line += ")"
            options_text_lines.append(option_line)
            
            builder.add(InlineKeyboardButton(text=f"Выбрать {i+1}", callback_data=f"show_recipe_{recipe.get('id')}"))
        
        builder.adjust(3) # Выравниваем кнопки по 3 в ряд

        return {
            "text": "\n".join(options_text_lines),
            "found_terms": [], # Термины не ищем на этапе выбора опций
            "reply_markup": builder.as_markup() # Возвращаем клавиатуру
        }

    else: # status == "none" - когда совсем ничего не найдено
        return {
            "text": phrases.get("rejection_phrases", {}).get("no_recipe_found", "Моя извращенная фантазия не может придумать ничего путного из этого набора. Попробуй другую комбинацию или добавь что-то еще."),
            "found_terms": []
        }